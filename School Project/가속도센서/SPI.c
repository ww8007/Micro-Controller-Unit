...
void Display_Process(int16 *pBuf)
{
		...
		if (pBuf[0] < 0)
		{	ACC_X = abs(pBuf[0]);
			LCD_DisplayChar(1,3,'-');//16진수표시
			LCD_DisplayChar(1,10,'-');// g 표시
		}
		else 
		{   ACC_X = pBuf[0];
			LCD_DisplayChar(1,3,'+');
		...
		sprintf(str, "%5x", ACC_X);	//X AXIS (16비트: 0~65535/2)
        LCD_DisplayText(1,4,str);
    	ACC_X= 100* ACC_X / 0x4009;
		LCD_DisplayChar(1,11,ACC_X/100 + 0x30);
		LCD_DisplayChar(1,12,'.');
		LCD_DisplayChar(1,13,ACC_X%100/10 + 0x30);
		LCD_DisplayChar(1,14,ACC_X%10 + 0x30);
        ...
        sprintf(str, "%5x", ACC_X);	//Y AXIS (16비트: 0~65535/2)
        LCD_DisplayText(2,4,str);
    	ACC_X= 100* ACC_X / 0x4009;
		LCD_DisplayChar(2,11,ACC_X/100 + 0x30);
		LCD_DisplayChar(2,12,'.');
		LCD_DisplayChar(2,13,ACC_X%100/10 + 0x30);
		LCD_DisplayChar(2,14,ACC_X%10 + 0x30);
        ...
        sprintf(str, "%5x", ACC_X);	//Z AXIS (16비트: 0~65535/2)
        LCD_DisplayText(3,4,str);
    	ACC_X= 100* ACC_X / 0x4009;
		LCD_DisplayChar(3,11,ACC_X/100 + 0x30);
		LCD_DisplayChar(3,12,'.');
		LCD_DisplayChar(3,13,ACC_X%100/10 + 0x30);
		LCD_DisplayChar(3,14,ACC_X%10 + 0x30);
...
void DisplayTitle(void)
{
    ...
    LCD_SetBackColor(RGB_GREEN);
    LCD_DisplayText(5, 0, "2015132012 JDH");
