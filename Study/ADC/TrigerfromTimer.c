...
{   	// ADC2: PA1(pin 41)
	...
	ADC2->CR2 |= (0X07<<24);	// EXTSEL[3:0]: ADC_ExternalTrig (TIM3_CC1)
...
// Clock Enable : GPIOA & TIMER5RCC->AHB1ENR	|= (1<<0);	// GPIOA Enable
...
// PA2을 출력설정하고 Alternate function(TIM5_CH3)으로 사용 선언 
	...
	TIM3->PSC = 8400-1;	// Prescaler 84MHz/840 = 0.1MHz (10us)
	TIM3->ARR = 4000-1;	// Auto reload  : 10us * 50K = 500ms(period)
	...
	TIM3->CCER |= (1<<8);	// CC3E=1: CC3 channel Output Enable
				// OC3(TIM3_CH3) Active: 해당핀을 통해 신호출력
	TIM3->CCER |= (1<<1);	// CC1P active high
	...
	TIM3->CCMR1 |= (3<<4);	// OC3M=0b011: Output Compare 3 Mode : toggle
 	...
 	TIM3->CCR3 = 1000;	// TIM3 CCR3 TIM3_Pulse

	////////////////////////////////
	// Disable TIM3 CC3 interrupt
